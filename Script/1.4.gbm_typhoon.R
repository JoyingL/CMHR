...
## (0) data ---------------------------------------------------
tf <- read_csv("Data/lightgbm/Book4.csv") %>% dplyr::select(cyclone)
...
# 归一化权重
tf %<>%
  mutate(tf = case_when(cyclone != 0 ~ 1, cyclone == 0 ~ 0)) %>%
  mutate(tf = as.factor(tf)) %>%
  # mutate(weights = rescale(cyclone)) %>%
  dplyr::select(-1) %>% cbind(nomissing)
glimpse(tf)

table(tf$tf) # 221756  19576
range(tf$weights)



## (1) task -------------------------------------------------
task <- as_task_classif(tf, target = "tf") # for missing data impute and ...
...
### 1.feature selection ###
drop_features <- c("plancurvature", "aspect")
task$select(setdiff(task$feature_names, drop_features)) # 删除变量


### 2. undersample majority class ###
prep_under <- po("classbalancing", id = "undersample", adjust = "major",
                 reference = "major", shuffle = F, ratio = 19576 / 221756)
...
# graph_learner %>% write_rds("Data/lightgbm/graph_learner_tf.rds")
# graph_learner <- read_rds("Data/lightgbm/graph_learner_tf.rds")


## (8) validation ---------------------------------------------
(predictions$score(msr("classif.auc")) -> dd1)
# classif.auc 
#   0.8814704  
(predictions$score(msr("classif.acc")) -> dd2)
# classif.acc 
#   0.8140644 
(predictions$score(msr("classif.recall")) -> dd3)
# classif.recall 
#      0.7454453
...
autoplot(predictions, type = "roc") + labs(subtitle = "cyclone")
ggsave("Output/SpFig/lightgbm/roc_cyclone.pdf", scale = 2)
...
tsk_tf <- as_task_classif(tf, target = "tf")
tsk_tf$set_col_roles("weights", roles = "weight")
tsk_tf


### Calculating Filter Values ###
flt_gain <- flt("information_gain")
flt_gain$calculate(tsk_tf)
as.data.table(flt_gain)

# flt_jmim <- flt("jmim")
# flt_jmim$calculate(tsk_tf)
...
flt_importance1$calculate(tsk_tf)

as.data.table(flt_importance1)
autoplot(flt_importance1) + labs(subtitle = "cyclone")
ggsave("Output/SpFig/lightgbm/fs_cyclone.pdf", scale = 2)
...
lrn_exp <- explain_mlr3(lrn_rf,
                        data = tf %>% dplyr::select(-c(weights, tf)),
                        y = as.numeric(as.character(tf$tf)),
...
# # 随机抽样，平衡样本
# dz_0 = tf %>% filter(tf == 0) # 240348
# dz_1 = tf %>% filter(tf != 0) # 8975
#
# set.seed(888)
...
as.data.table(flt_gain) %>% ggplot() +
  ...
  labs(
    ...
    title = "Feature Selection", subtitle = "cyclone"
  )
ggsave("Output/SpFig/lightgbm/fs_gain_tf.pdf", scale = 2)
