...
## (0) data ---------------------------------------------------
hp <- read_csv("Data/lightgbm/Book4.csv") %>% dplyr::select(landslide)
...
# 归一化权重
hp %<>%
  mutate(hp = case_when(landslide != 0 ~ 1, landslide == 0 ~ 0)) %>%
  mutate(hp = as.factor(hp)) %>%
  mutate(weights = rescale(landslide)) %>%
  dplyr::select(-1) %>%
  cbind(nomissing)
glimpse(hp)

table(hp$hp) # 237804   3528
range(hp$weights)



## (1) task -------------------------------------------------
task <- as_task_classif(hp, target = "hp") # for missing data impute and ...
...
prep_under <- po("classbalancing",
  ...
  reference = "major", shuffle = F, ratio = 3528 / 237804
)
...
# graph_learner %>% write_rds("Data/lightgbm/graph_learner_hp.rds")


## (8) validation ---------------------------------------------
(predictions$score(msr("classif.auc")) -> bb1)
# classif.auc
#   0.8893693 
(predictions$score(msr("classif.acc")) -> bb2)
# classif.acc
#   0.8100189 
(predictions$score(msr("classif.recall")) -> bb3)
# classif.recall
#      0.7429112 
...
autoplot(predictions, type = "roc") + labs(subtitle = "landslide")
ggsave("Output/SpFig/lightgbm/roc_landslide.pdf", scale = 2)
...
tsk_hp <- as_task_classif(hp, target = "hp")
tsk_hp$set_col_roles("weights", roles = "weight")
tsk_hp


### Calculating Filter Values ###
flt_gain <- flt("information_gain")
flt_gain$calculate(tsk_hp)
...
flt_importance1$calculate(tsk_hp)

as.data.table(flt_importance1)
autoplot(flt_importance1) + labs(subtitle = "landslide")
ggsave("Output/SpFig/lightgbm/fs_landslide.pdf", scale = 2)
...
lrn_exp <- explain_mlr3(lrn_rf,
  data = hp %>% dplyr::select(-c(weights, hp)),
  y = as.numeric(as.character(hp$hp)),
...
# # 随机抽样，平衡样本
# dz_0 = hp %>% filter(hp == 0) # 240348
# dz_1 = hp %>% filter(hp != 0) # 8975
#
# set.seed(888)
...
as.data.table(flt_gain) %>% ggplot() +
  ...
  labs(
    ...
    title = "Feature Selection", subtitle = "landslide"
  )
ggsave("Output/SpFig/lightgbm/fs_gain_hp.pdf", scale = 2)
